...
./install.sh
```
This file will setup a Python virtual environment, as well as the document collection of wiki pages used by Watson.

> [!IMPORTANT]
> To run this project as-designed, you should have access to a copy of the wiki document collection [`wiki-subset-20140602.tar.gz`](https://arizona.box.com/v/wikidata-csc483). This file will be unzipped to a folder `wiki` during install, to be used by Watson. If you cannot access the required `tar.gz`, you may create your own files in the format of `example/wiki-example.txt` to folder `wiki` and it should work the same.
...
```
```bash
make clean # Cleans venv, wiki, and temp files
```
