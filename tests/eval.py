from retrievers import Jeopardy
...
def eval_p_at_1(ir: Jeopardy, questions: list[tuple[str, str, list[str]]], k: int):
    pat1 = 0
...
def eval_mrr(ir: Jeopardy, questions: list[tuple[str, str, list[str]]], k: int):
    mrr = 0.0
    for category, clue, answers in questions:
        titles = ir.search(construct_query(category, clue), k=k)
